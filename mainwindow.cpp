...
void MainWindow::paintEvent(QPaintEvent *event) {
    ...
    QPixmap pixmap(":/theme0.background2.jpg");
...
void MainWindow::easy_return_to_main(){
    delete Easyclock;
    Easyclock = nullptr;
    ui->sign_in_button->show();
    ui->register_button->show();
...
void MainWindow::on_visitor_clicked(){
    if(!Easyclock)
    {
        Easyclock=new easyclock(this);
        connect(Easyclock,&easyclock::returnToMain,this,&MainWindow::easy_return_to_main);
    }
    Easyclock->show();
    ui->sign_in_button->hide();
    ui->register_button->hide();
